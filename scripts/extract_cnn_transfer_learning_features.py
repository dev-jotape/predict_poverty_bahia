...
np.save('../model/x_all.npy', x_all)
np.save('../model/y_all.npy', y_all)

# del(a, all_figures, class_1_files, class_2_files, class_3_files, i, idx, 
#     path_1, path_2, path_3, t1, t2, trainData, trainLabels)
# gc.collect()

# x_all = np.load('../model/x_all.npy')
# y_all = np.load('../model/y_all2.npy')
...
income_city_files = pd.read_csv('../model/income_city_file.txt')
...
for city in income_city_files['city_code'].unique():
    ...
    for i in range(df_filter.shape[0]):
        img = utils.load_img(df_filter.iloc[i, [2]][0])
        x = utils.img_to_array(img)
        x = np.expand_dims(x, axis=0)
        x = preprocess_input(x)
...
np.savetxt('../model/google_image_features_cnn_transfer.csv', features_final)
